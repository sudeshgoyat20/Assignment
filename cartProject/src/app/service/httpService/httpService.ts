...
export class HttpServiceProvider {
  userToken;
  hideSpinner;
  baseUrl='https://fanappapi.azurewebsites.net/'
 ...
 // }
  ...
  login(params) {
    ...
    return this.http.post(this.baseUrl+"api/auth/Login",params,options);
  ...
  getUsers(token){
    // return this.http.get( this.baseUrl+"api/user/list?pageNo=1&pageSize=50");
    let params={
      "pageNo":1,
      "pageSize":50

    }
  //  return this.http.post( this.baseUrl+"api/user/list?pageNo=1&pageSize=50",params);
    let headers = new HttpHeaders({
      'Content-Type': 'application/json',
     'Authorization':"Bearer"+ ' '+token
   });
  let options = { headers: headers };
  return this.http.post(this.baseUrl+"api/user/list?pageNo=1&pageSize=50",params,options);
